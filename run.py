...
def list_rooms(viewer_id=None):
    ...
    روم‌های خصوصی فقط برای کسایی نشون داده می‌شن که:
    ...
    for r in rows:
        ...
        out.append({
            ...
            "icon": r["icon"], "is_private": is_private,
...
async def join_room_internal(ws, sess, room_id):
    ...
    await send_to(ws, {
        "type": "joined_room", "room": {
            ...
            "invite_code": room["invite_code"] if (can_manage and room["kind"] == "private") else None,
        },
...
async def on_create_room(ws, sess, data):
    ...
    invite_code = generate_unique_invite_code(conn) if kind == "private" else None
    with db.tx() as c:
        cur = c.execute(
            "INSERT INTO rooms(name, slug, kind, owner_id, invite_code, icon, created_at) VALUES (?,?,?,?,?,?,?)",
            (name, slug, kind, sess["user_id"], invite_code, icon, now()),
        )
...
async def on_room_update(ws, sess, data):
    ...
    await send_to(ws, {"type": "room_update_result", "ok": True, "name": updated["name"], "icon": updated["icon"]})
    await broadcast_room(room_id, {"type": "room_renamed", "room_id": room_id, "name": updated["name"], "icon": updated["icon"]})
    await push_room_list()
