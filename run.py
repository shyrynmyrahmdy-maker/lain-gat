...
def public_user(row):
    return {
        ...
        "score": row["score"],
        "rank": db.rank_for(row["message_count"]),
        "next_rank": db.next_rank_for(row["message_count"]),
...
def serialize_message(row, viewer_id=None):
    ...
    return {
        ...
        "rank": db.rank_for(user["message_count"]) if user else None,
        "text": row["text"] or "",
...
def list_rooms():
    ...
    for r in rows:
        ...
        out.append({
            ...
            "icon": r["icon"], "is_private": bool(r["password_hash"]),
...
async def push_room_list():
    payload = {"type": "room_list", "rooms": list_rooms()}
...
async def push_leaderboard():
    conn = db.get_conn()
    rows = conn.execute(
        "SELECT * FROM users WHERE banned=0 ORDER BY score DESC, message_count DESC LIMIT 20"
    ).fetchall()
    board = [public_user(r) for r in rows]
    await broadcast_all({"type": "leaderboard", "board": board})
...
async def on_login(ws, sess, data):
    ...
    await send_to(ws, {"type": "room_list", "rooms": list_rooms()})
...
async def on_join_room(ws, sess, data):
    ...
    if not room:
        ...
        return
    if room["password_hash"] and not db.verify_password(password, room["password_hash"]):
        # owners and admins skip password check
        if room["owner_id"] != sess["user_id"] and not is_admin(sess["user_id"]):
            await send_to(ws, {"type": "join_room_result", "ok": False, "msg": "WRONG_ROOM_PASSWORD"})
            return
...
async def on_create_room(ws, sess, data):
    ...
    kind = data.get("kind", "public")
    password = data.get("password", "")
    icon = str(data.get("icon", "💬"))[:4] or "💬"
    if kind not in ("public", "game", "private"):
        kind = "public"
    if not name:
    ...
    pw_hash = db.hash_password(password) if (kind == "private" and password) else None
    with db.tx() as c:
        cur = c.execute(
            "INSERT INTO rooms(name, slug, kind, owner_id, password_hash, icon, created_at) VALUES (?,?,?,?,?,?,?)",
            (name, slug, kind, sess["user_id"], pw_hash, icon, now()),
        )
        room_id = cur.lastrowid
    await send_to(ws, {"type": "create_room_result", "ok": True, "room_id": room_id})
    await push_room_list()
    await join_room_internal(ws, sess, room_id)
...
def award_points(uid, delta=1):
    with db.tx() as c:
        c.execute(
            "UPDATE users SET message_count = message_count + 1, score = score + ? WHERE id=?",
            (delta, uid),
        )
...
async def http_upload(request):
    ...
    target_dir = UPLOADS_AVATAR if kind == "avatar" else UPLOADS_MEDIA
    new_name = f"{uuid.uuid4().hex}{ext}"
    (target_dir / new_name).write_bytes(file_bytes)

    sub = "avatars" if kind == "avatar" else "media"
